...
var RECIPIENT = "johnstylianou1952@gmail.com";
...
var contactRouter = router({
  sendEnquiry: publicProcedure.input(
    z2.object({
    ...
    if (!transport) {
      console.log("\n\u{1F4E7} [CONTACT FORM \u2014 no SMTP configured, logging only]");
      console.log(text2);
...
// server/pricesProxy.ts
var BASE_GOLD_USD = 4029;
var BASE_SILVER_USD = 56.82;
var BASE_AUD_RATE = 1.45;
var lastGoldUSD = BASE_GOLD_USD;
var lastSilverUSD = BASE_SILVER_USD;
var lastAudRate = BASE_AUD_RATE;
...
function registerPricesProxy(app) {
  app.get("/api/prices", async (_req, res) => {
    try {
      ...
      console.warn("[prices] Live fetch failed, using simulated drift:", err.message);
      const drift = () => 1 + (Math.random() - 0.5) * 8e-4;
